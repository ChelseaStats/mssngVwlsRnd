...
class player:
    ...
    def get_guess_player_string(self):
        if(self.apps == 0):
            ...
            if(self.active == 1):
                return f"#GuessThePlayer This player made his debut for #Chelsea on {debut_string}" \
                f" making {self.apps} appearances so far, scoring {goals_string}. #CFC"
            else:
                return f"#GuessThePlayer This formerblue made his debut for #Chelsea on {debut_string}" \
                f" making a total of {self.apps} appearances, scoring {goals_string}. #CFC"
